				...
				<tr>
				  ...
				  <td>
					...
					if($rfqs[$i]['bid_id']==-1){ //if cancelled listing
						?>
						<input type="button" class="btn btn-sm" onclick="self.location='<?php echo site_url($class."/rfq")."/".$rfqs[$i]['id']."/bids";; ?>'" value="Cancelled: <?php echo date("M d,Y", strtotime($rfqs[$i]['datecancelled']));?>" />
						
						<?php
					}
					else if($rfqs[$i]['bid_id']==0){
						?>
						<input type="button" class="btn btn-sm" onclick="self.location='<?php echo site_url($class."/rfq")."/".$rfqs[$i]['id']."/bids";; ?>'" value="Bids" />
						<input type="button" class="btn btn-sm" style="background:red; color: white" onclick="self.location='<?php echo site_url($class."/rfq")."/".$rfqs[$i]['id']."/cancel";; ?>'" value="Cancel RFQ" />
						<?php
					}
					else{
						?>
